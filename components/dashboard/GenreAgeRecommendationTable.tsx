    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          combo&rsquo;s audience actually is.
        </CardDescription>
      </CardHeader>
            ...
            <TableBody>
              {rows.map((r, i) => (
                <TableRow key={`${r.genre}|${r.ageRecommendation}`}>
                  ...
                  <TableCell className="font-medium text-foreground">{r.genre}</TableCell>
                  <TableCell>
                    <Badge variant="secondary" className="font-normal">
                      {r.ageRecommendation}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right tabular-nums">{fmt.format(r.gameCount)}</TableCell>
                  <TableCell className="text-right tabular-nums text-muted-foreground">
                    {pct.format(r.shareOfPlayers)}
                  </TableCell>
                  <TableCell className="text-right tabular-nums">
                    {r.meanSessionMinutes === null ? "—" : r.meanSessionMinutes.toFixed(1)}
                  </TableCell>
